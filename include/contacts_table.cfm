<style>
    .exportcontacts, .updatetag, .updatesystem, .addrelationship,
    .importhistory,.batchdelete,.searchtag, .import {
    color: #fff;
...
<div class="table-responsive" id="<cfoutput>#contacts_table#_container</cfoutput>">
    <table id="<cfoutput>#contacts_table#</cfoutput>" 
           class="table display nowrap table-striped dataTable w-100 dtr-inline dt-checkboxes-select dt-responsive">
        <thead>
            <tr>
...
<script type="text/javascript">
    ...
    var table = $('#
    <cfoutput>#contacts_table#</cfoutput>
    ').DataTable({
    "pageLength": 
    <cfoutput>#userdefRows#</cfoutput>
    ,
    stateSave: false,
    dom: 'Bfrtip',
    responsive: {
    ...
    ajax: {
    url: '/include/contacts_ss.cfm?contacts_table=
    <cfoutput>#contacts_table#</cfoutput>
    &userid=
    <cfoutput>#session.userid#</cfoutput>
    &bytag=
    <cfoutput>#bytag#</cfoutput>
    &byimport=
    <cfoutput>#byimport#</cfoutput>
    ',
    type: 'post'
    },
    buttons: [
    ...
    $('#
    <cfoutput>#contacts_table#_container</cfoutput>
    ').css( 'display', 'table' );
    ...
    $('#
    <cfoutput>#contacts_table#_container</cfoutput>
    ').on('click','input[type="Text"]', function(event){
...
<script>
    ...
    $("#batchbutton_
    <cfoutput>#contacts_table#</cfoutput>
    :visible").fadeOut(); //if there are none checked, hide only visible elements
    } else {
    $("#batchbutton_
    <cfoutput>#contacts_table#</cfoutput>
    :hidden").fadeIn(); //otherwise (some are selected) fadeIn - if the div is hidden.
    }
    };
    countChecked();
